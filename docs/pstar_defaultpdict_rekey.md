# [`pstar`](./pstar.md).[`defaultpdict`](./pstar_defaultpdict.md).`rekey(self, map_or_fn=None, inplace=False, **kw)`

Change the keys of `self` or a copy while keeping the same values.

Convenience method for renaming keys in a [`defaultpdict`](./pstar_defaultpdict.md). Passing a `dict` mapping
...
>             `False`, returns a new [`defaultpdict`](./pstar_defaultpdict.md).
...
>    `self` if `inplace` evaluates to `True`, otherwise a new [`defaultpdict`](./pstar_defaultpdict.md). The keys will
...
## [Source](../pstar/pstar.py#L854-L916)